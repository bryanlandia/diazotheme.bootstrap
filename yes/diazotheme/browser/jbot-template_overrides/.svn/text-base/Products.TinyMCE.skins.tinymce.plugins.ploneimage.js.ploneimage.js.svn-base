        ...
        tinymce.util.XHR.send({
            ...
            success : function(text) {
                ...
                if (data.thumb == "") {
                    ...
                    document.getElementById ('previewimagecontainer').innerHTML = '<img src="' + data.thumb + '" style="border:0" />';
                }
                ...
                /* YES! customization here: fill in the href for the edit 
                Image content item link */
                document.getElementById('edit_image_item_link').href = path +'/edit#';
                /* end YES! customization */
                ...
                /* prep the image item editor form overlay */
                jQuery(function($){

                  $('a#edit_image_item_link').prepOverlay(
                      {
                          subtype: 'ajax',
                          width: '700',
                          filter: 'div#content',
                          formselector: '#image-base-edit',
                          noform: 'close',
                          closeselector: "#image-base-edit input[name='form\\.button\\.cancel']",
                          //initialize TinyMCE for wysiwyg in ajax-loaded form
                          config: {
                            onLoad:function () {
                              var initfunc = kukit && kukit.actionsGlobalRegistry.get("init-tinymce");
                              if (initfunc && $('#image-base-edit .mce_editable')) {                              
                                initfunc({node:{id:'credit'}});
                                // work around duplicate 'caption' ids in popup... we want textarea element
                                $('textarea#caption').attr('id','imgcaption');
                                initfunc({node:{id:'imgcaption'}});
                              }
                            }
                          }
                      }
                    ...
                    for (var i = 0; i < data.items.length; i++) {
                        ...
                        if (data.items[i].is_folderish) {
                            if (data.items[i].icon.length) {
                                html += '<img src="' + data.items[i].icon + '" style="border:0; margin-left: 17px" /> ';
                            }
                            ...
                            html += '"/> ';
                            if (data.items[i].icon.length) {
                                html += '<img src="' + data.items[i].icon + '" style="border:0"/> ';
                            }
