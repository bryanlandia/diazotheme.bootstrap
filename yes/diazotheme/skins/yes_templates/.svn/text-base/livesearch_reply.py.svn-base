...
results = catalog(SearchableText=r, portal_type=friendly_types, path=path, sort_on='effective', sort_order='descending')
...
if not results:
    ...
    if len(results)>limit:
        # add a more... row
        write('''<li class="LSRow">''')
        write( '<a href="%s" style="font-weight:normal">%s</a>' % ('search?SearchableText=' + searchterms + '&amp;sort_on=effective&amp;sort_order=descending', ts.translate(label_show_all)))
        write('''</li>''')
    write('''</ul>''')
    write('''</div>''')
    write('''</fieldset>''')
